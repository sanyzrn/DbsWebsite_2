  ...
  return (
    <section id="process" className="section-pad border-t border-line bg-surface">
      <div className="wrap">
        ...
        <ol className="dieline mt-8 md:mt-14 lg:mt-20" aria-label={t.process.title} tabIndex={0}>
          {steps.map((step, i) => (
            <li key={step.en} className="flex flex-col">
              <span className="tnum text-[15px] font-semibold text-accent">{nf.format(i + 1)}</span>
              <h3 className="display mt-10 text-[2rem] leading-none lg:mt-16 xl:mt-20 xl:text-[2.125rem]">{step.title}</h3>
