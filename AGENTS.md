...
- Contact form: posts JSON to same-origin `/api/contact.php`, which forwards to Telegram via a server-side bot token (GitHub Actions secrets → `dist/api/telegram-config.php` at deploy). Always keep the visible direct-email link.
