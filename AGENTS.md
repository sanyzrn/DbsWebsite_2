...
- Mobile: sections that would stack long (projects, disciplines, skills) use the `Slider` in `src/components/ui.tsx` below `lg`; the process dieline scrolls sideways. The bottom quick-nav (`FloatingQuickNav`) shows below `lg` only: closed by default, a registration-mark button that unfolds into the destinations.
- Accent colour is switchable (`src/lib/accent.ts`, `AccentPicker.tsx`): six ink-named spot colours set via `html[data-accent]`, persisted as `sz-accent`, applied pre-paint in `index.html`. Reflex Blue is the default (no attribute). Structural borders use the soft `--rule` token, never `--ink`.
